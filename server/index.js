...
app.post("/api/generate",async(req,res)=>{try{const{text,references=[],overrides={}}=req.body;if(!text?.trim())throw new Error("The imported document has no text.");const[translated,passages]=await Promise.all([translateDocument(text,{apiKey:process.env.GEMINI_API_KEY,model:process.env.GEMINI_MODEL||"gemini-3.6-flash"}),resolveBiblePassages(references,overrides,{esvToken:process.env.ESV_API_TOKEN})]);const pairedPassages=passages.map(p=>{const extracted=translated.scriptureEnglish?.[p.reference]?.trim();return extracted?{...p,english:{reference:p.reference,text:extracted,source:"Extracted from source handout"}}:p;});const seen=new Set();const blocks=(translated.blocks||[]).filter(block=>{if(block.type!=="scripture")return true;const reference=references.find(ref=>ref.toLowerCase()===String(block.reference||"").toLowerCase());if(!reference||seen.has(reference))return false;block.reference=reference;seen.add(reference);return true;});for(const reference of references)if(!seen.has(reference))blocks.unshift({type:"scripture",reference,english:"",chinese:""});res.json({...translated,blocks,passages:pairedPassages});}catch(error){res.status(500).json({error:error.message});}});
