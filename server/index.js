...
app.post("/api/generate",async(req,res)=>{try{const{text,references=[],overrides={},demo=false}=req.body;if(!text?.trim())throw new Error("The imported document has no text.");const[translated,passages]=await Promise.all([translateDocument(text,{apiKey:process.env.GEMINI_API_KEY,model:process.env.GEMINI_MODEL||"gemini-3.6-flash",demo}),resolveBiblePassages(references,overrides,{esvToken:process.env.ESV_API_TOKEN})]);res.json({...translated,passages});}catch(error){res.status(500).json({error:error.message});}});
